...
test.describe('Automation - Working With Elements', () => {
  ...
  test('handle tabs - example 1', async ({ page }) => {
    ...
    const [newTab] = await Promise.all([ //new tab is an array
      page.waitForEvent('popup'), //inbuilt function to wait for new tab
  ...
  test("flipkart - verify product display", async ({ page }) =>{
    ...
    await expect(newTab).toHaveURL(/apple-iphone-16-black-128-gb/) //partial url
